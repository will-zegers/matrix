...
| `ValuesIn(container)` and `ValuesIn(begin, end)` | Yields values from a C-style array, an STL-style container, or an iterator range `[begin, end)`. `container`, `cbegin`, and `end` can be expressions whose values are determined at run time.  |
